...
import {
    ...
    ScrollView, Platform,
    TouchableWithoutFeedback, ImageEditor, ImageStore,
} from 'react-native';
        ...
        if (height != null && width != null) {
            ...
            Image.getSize(url, (width, height) => {
                ImageEditor.cropImage(url, {
                    ...
                    const resize = await ImageResizer.createResizedImage(uri, width, height, 'JPEG', 80);
                    const file = await RNFetchBlob.fs.readFile(resize.path, 'base64');
                    console.log(file);
                    await this.addImageContent(`data:image/jpeg;base64,${file}`, id, height, width);
                }, err => console.log(err));
            });
        }
    }
